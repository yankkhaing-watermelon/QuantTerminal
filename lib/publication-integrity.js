  ...
  if (Array.isArray(run.portfolio)) {
    ...
    for (const row of run.portfolio) {
      ...
      if (row.target_weight !== undefined && row.target_weight !== null) {
        ...
        if (weight < 0 || weight > 15.000001) throw new PublicationIntegrityError(`target_weight_out_of_range:${symbol}:${weight}`);
        targetWeight += weight;
      }
    }
    if (targetWeight > 100.000001) throw new PublicationIntegrityError(`portfolio_target_weight_exceeds_100:${targetWeight}`);
  }
