...
function download(result, key, runId) {
  const fields = ["symbol", "name", "sector", "signal_date", "entry_date", "exit_date", "entry", "exit", "shares", "initial_stop", "pnl", "r", "fees", "reason", "initial_risk", "entry_fee", "signal_atr", "signal_turnover", "signal_breadth", "signal_rs_percentile", "signal_momentum", "stop_trigger", "exit_fills"];
  const quote = (value) => `"${(typeof value === "number" ? String(value) : String(value ?? "").replace(/^[=+\-@\t\r]/, "'$&")).replaceAll('"', '""')}"`;
  const csv = [fields.join(","), ...result.trades.map((row) => fields.map((field) => quote(row[field] && typeof row[field] === "object" ? JSON.stringify(row[field]) : row[field])).join(","))].join("\r\n");
...
export default function Astra({ settings = INITIAL, setSettings, sharedJob }) {
  ...
  return <div className="astra">
    ...
    {payload && <>
      ...
      {view === "signals" && <section className="astra-panel"><div className="astra-panel-head"><h4>{candidates.length} {payload.signal_window_sessions === 5 ? "fresh & recent signals · 5 sessions" : "qualifying signals"}</h4><input aria-label="Search Astra stocks" placeholder="Code or company" value={search} onChange={(e) => setSearch(e.target.value)}/></div><p>Fresh signals are from the latest completed session. Recent signals show earlier triggers, not new entry instructions. Prices and reference stops below are from the signal date. Backtests enter only after the original trigger; older signals are not repeatedly entered.</p>
        {!candidates.length ? <p className="astra-empty">No stocks match this strategy and search for the published session. Cash is a valid outcome.</p> : <div className="astra-candidates">{candidates.map((r) => <article key={r.symbol}><div><small>#{r.rank} · {r.symbol} · {r.tv_symbol}</small><h4>{r.name}</h4><span>{r.sector}</span></div><dl><dt>Signal date</dt><dd>{r.signal_date} · {r.signal_status || "Fresh"}{r.signal_age_sessions > 0 ? ` · ${r.signal_age_sessions} sessions ago` : ""}</dd><dt>Signal close</dt><dd>{money(r.close)}</dd><dt>Reference stop</dt><dd>{money(r.reference_stop)}</dd><dt>126D momentum</dt><dd>{pct(r.momentum_pct)}</dd><dt>RS percentile</dt><dd>{n(r.rs_percentile)}</dd><dt>Median turnover</dt><dd>{money(r.median_turnover)}</dd></dl></article>)}</div>}
      </section>}
      {view === "backtest" && <>
