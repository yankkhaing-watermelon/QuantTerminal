...
function StockTable({ rows, compact = false }) {
  ...
  return <div className="table-wrap"><table className={compact ? "compact" : ""}><thead><tr><th>{head("symbol", "Stock")}</th><th>{head("quant_score", "Quant")}</th><th>{head("expected_edge", "Edge")}</th><th>{head("confidence", "Confidence")}</th><th>{head("close", "Close")}</th><th>{head("rs_20d", "RS 20D")}</th><th>Decision</th></tr></thead><tbody>{sorted.map((row, index) => <tr key={row.symbol}><td><div className="stock"><b>{row.symbol}</b><span>{row.name || row.sector || `Rank ${index + 1}`}</span></div></td><td><span className={`score ${Number(row.quant_score) >= 70 ? "elite" : ""}`}>{num(row.quant_score, 0)}</span></td><td className={tone(row.expected_edge)}>{pct(row.expected_edge, 2)}</td><td>{pct(row.confidence, 0)}</td><td>RM {num(row.close, Number(row.close) < 1 ? 3 : 2)}</td><td className={tone(row.rs_20d)}>{pct(row.rs_20d, 2)}</td><td><Badge kind={String(row.action || "WATCH").toLowerCase()}>{row.action || "WATCH"}</Badge></td></tr>)}</tbody></table></div>;
}
...
function Backtest({ data }) {
  ...
  return <><section className="metric-grid"><Metric label="Total trades" value={backtest.total_trades || 0}/><Metric label="Win rate" value={pct(backtest.win_rate)}/><Metric label="Expectancy" value={pct(backtest.expectancy, 2)} valueClass={tone(backtest.expectancy)}/><Metric label="Max drawdown" value={pct(backtest.max_drawdown, 2)} valueClass="down"/></section><section className="panel"><div className="panel-head"><div><span className="eyebrow">WALK-FORWARD RESULTS</span><h3>Score-decile validation</h3></div></div>{backtest.groups?.length ? <div className="backtest-grid">{backtest.groups.map((group) => <article key={group.name}><h4>{group.name}</h4><strong>{pct(group.win_rate)}</strong><span>{group.trades} trades</span><div className="mini-stats"><b className={tone(group.expectancy)}>E {pct(group.expectancy, 2)}</b><b>PF {num(group.profit_factor, 2)}</b></div></article>)}</div> : <Empty title="No stored backtest payload" text="The engine will publish score-decile and exit-strategy validation after sufficient history is available."/>}</section></>;
}
